...
export function OtherIncomeEditModal({ record, onClose, onSuccess }: OtherIncomeEditModalProps) {
  const [formData, setFormData] = useState<Partial<OtherIncome>>(record);
  ...
  const handleVendorCodeChange = async (vendorCode: string) => {
    ...
    try {
      ...
      console.error('Í±∞ÎûòÏ≤??ïÎ≥¥ Ï°∞Ìöå ?§Î•ò:', err);
    }
  };
    ...
    try {
      ...
      // ???åÏùº ?ÖÎ°ú??      if (invoiceFile) {
        ...
        if (!uploadResponse.ok || !uploadData.success) {
          const errorMsg = uploadData.error || uploadData.details?.message || '?åÏùº ?ÖÎ°ú?úÏóê ?§Ìå®?àÏäµ?àÎã§.';
          console.error('?åÏùº ?ÖÎ°ú???§Ìå®:', uploadData);
      ...
      if (!response.ok) {
        const data = await response.json();
        throw new Error(data.error || '?òÏ†ï???§Ìå®?àÏäµ?àÎã§.');
      }
    ...
    <div className="fixed inset-0 z-50 flex items-center justify-center bg-black bg-opacity-50 overflow-y-auto p-4">
      <div className="bg-white rounded-lg shadow-xl w-full max-w-6xl my-8 max-h-[90vh] overflow-hidden flex flex-col">
        <div className="sticky top-0 bg-white border-b p-6 flex items-center justify-between z-10">
          <h2 className="text-xl font-semibold">?ÖÍ∏à ?ïÎ≥¥ ?òÏ†ï</h2>
          <button
            onClick={onClose}
              ...
              <div className="space-y-2">
                {invoiceFileUrl && (
                  <div className="flex items-center gap-2 mb-2">
                    <img src={invoiceFileUrl} alt="?∏Í∏àÍ≥ÑÏÇ∞?? className="max-w-xs max-h-32 border rounded" />
                    <button
                      type="button"
                      onClick={removeFile}
                ...
                <label className="flex items-center gap-2 px-4 py-2 border border-gray-300 rounded-md cursor-pointer hover:bg-gray-50 w-fit">
                  ...
                  <span className="text-sm">{invoiceFile ? '?åÏùº Î≥ÄÍ≤? : '?åÏùº ?†ÌÉù'}</span>
                  <input
                    type="file"
                    accept="image/*"
