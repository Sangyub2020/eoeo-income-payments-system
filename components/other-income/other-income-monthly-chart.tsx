...
export function OtherIncomeMonthlyChart() {
  ...
  useEffect(() => {
    ...
    const savedExcluded = localStorage.getItem('other_income_excluded_ids');
    if (savedExcluded) {
  ...
  useEffect(() => {
    if (excludedIds.size > 0) {
      localStorage.setItem('other_income_excluded_ids', JSON.stringify(Array.from(excludedIds)));
    } else {
      localStorage.removeItem('other_income_excluded_ids');
    }
  }, [excludedIds]);
