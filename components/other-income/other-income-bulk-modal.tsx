...
export function OtherIncomeBulkModal({ isOpen, onClose, onSuccess }: OtherIncomeBulkModalProps) {
  ...
  const parseCsvText = (text: string, categoryToCodeMap: Map<string, string>): Partial<OtherIncome>[] => {
    ...
    try {
      ...
      for (let i = 1; i < allLines.length; i++) {
        ...
        const record: Partial<OtherIncome> = {
        ...
        if (record.projectCode) {
          ...
          if (project) {
            ...
            if (!enriched.projectName) {
              ...
              console.log(`ℹ️ 프로젝트 이름이 이미 있으므로 자동 채우기 건너뜀: "${enriched.projectName}"`);
            }
          }
        }
  ...
  const handleSubmit = async () => {
    ...
    try {
      ...
      const response = await fetch('/api/income-records/bulk', {
        ...
        body: JSON.stringify({ team: 'other_income', records: recordsToSubmit }),
      });
