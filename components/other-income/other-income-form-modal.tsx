...
export function OtherIncomeFormModal({ isOpen, onClose, onSuccess }: OtherIncomeFormModalProps) {
  const [formData, setFormData] = useState<Partial<OtherIncome>>({});
  ...
  const handleSubmit = async (e: React.FormEvent) => {
    ...
    try {
      ...
      if (!response.ok) {
        const data = await response.json();
        throw new Error(data.error || '?ÖÍ∏à ?ïÎ≥¥ ?±Î°ù???§Ìå®?àÏäµ?àÎã§.');
      }
    ...
    <div className="fixed inset-0 z-50 flex items-center justify-center bg-black bg-opacity-50 overflow-y-auto p-4">
      <div className="bg-white rounded-lg shadow-xl w-full max-w-6xl my-8 max-h-[90vh] overflow-y-auto">
        <div className="sticky top-0 bg-white border-b p-6 flex items-center justify-between z-10">
          <h2 className="text-xl font-semibold">?ÖÍ∏à ?ïÎ≥¥ ?±Î°ù</h2>
          <button
            onClick={onClose}
          ...
          <div className="grid grid-cols-3 gap-4">
            ...
            <div className="col-span-2">
              ...
              <div className="space-y-2">
                ...
                {invoiceFileUrl && (
                  <div className="mt-2">
                    <img src={invoiceFileUrl} alt="?∏Í∏àÍ≥ÑÏÇ∞???¨Î≥∏" className="max-w-xs max-h-48 border rounded" />
