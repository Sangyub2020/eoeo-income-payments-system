...
export function VendorBulkForm({ onSuccess }: VendorBulkFormProps) {
  ...
  const parseBulkText = (text: string): Omit<Vendor, 'id' | 'createdAt' | 'updatedAt'>[] => {
    ...
    for (const line of lines) {
      ...
      if (parts.length >= 2) {
        vendors.push({
          code: parts[0] || '',
          name: parts[1] || '',
          businessNumber: parts[2] || '',
          invoiceEmail: parts[3] || '',
        });
      }
    }
  ...
  const handleSubmit = async (e: React.FormEvent) => {
    ...
    try {
      const vendors = parseBulkText(bulkText);
      
      if (vendors.length === 0) {
        throw new Error('등록할 거래처 정보가 없습니다.');
      }

      // 필수 필드 검증
      const invalidVendors = vendors.filter(v => !v.code || !v.name);
      if (invalidVendors.length > 0) {
        throw new Error('모든 거래처는 코드와 거래처명이 필수입니다.');
      }
      ...
      if (data.result && data.result.failed > 0 && data.result.errors.length > 0) {
        ...
        // 실패한 항목만 남기기
        const vendors = parseBulkText(bulkText);
        const failedVendors = vendors.filter(v => failedCodes.has(v.code));
        
        if (failedVendors.length > 0) {
          const failedText = failedVendors
            .map(v => {
              const parts = [v.code, v.name];
              if (v.businessNumber) parts.push(v.businessNumber);
              if (v.invoiceEmail) parts.push(v.invoiceEmail);
              return parts.join('\t'); // 탭으로 구분
            })
            .join('\n');
          
          setBulkText(failedText);
        } else {
          // 코드 매칭 실패 시 원본 텍스트 유지
        ...
        <div className="flex justify-end gap-2 pt-4">
          ...
          {process.env.NODE_ENV === 'development' && (
            <div className="text-xs text-gray-400 mt-2">
              텍스트 길이: {bulkText.length}, hasText: {hasText ? 'true' : 'false'}, isSubmitting: {isSubmitting ? 'true' : 'false'}
            </div>
          )}
        </div>
      </form>
