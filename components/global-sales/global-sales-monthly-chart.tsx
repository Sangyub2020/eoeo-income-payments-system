...
export function GlobalSalesMonthlyChart() {
  ...
  useEffect(() => {
    ...
    const savedExcluded = localStorage.getItem('global_sales_excluded_ids');
    if (savedExcluded) {
  ...
  useEffect(() => {
    if (excludedIds.size > 0) {
      localStorage.setItem('global_sales_excluded_ids', JSON.stringify(Array.from(excludedIds)));
    } else {
      localStorage.removeItem('global_sales_excluded_ids');
    }
  }, [excludedIds]);
    ...
    try {
      ...
      if (data.success) {
        ...
        if (data.data && data.data.length > 0) {
          ...
          data.data.forEach((record: GlobalSalesTeam) => {
