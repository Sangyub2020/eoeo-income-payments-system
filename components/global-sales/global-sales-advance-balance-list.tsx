...
interface GlobalSalesAdvanceBalanceListProps {
  onSuccess?: () => void;
}

export function GlobalSalesAdvanceBalanceList({ onSuccess }: GlobalSalesAdvanceBalanceListProps) {
  ...
  const fetchRecords = async () => {
    ...
    try {
      const response = await fetch(`/api/income-records?team=global_sales&_t=${Date.now()}`, {
        cache: 'no-store',
      });
      if (!response.ok) {
  ...
  // 입금여부 계산 함수
  const getDepositStatus = useCallback((record: GlobalSalesTeam): '입금완료' | '입금예정' | '입금지연' => {
  ...
  // 선/잔금 경고 체크 함수
  const checkAdvanceBalanceWarning = useCallback((record: GlobalSalesTeam, allRecords: GlobalSalesTeam[]): boolean => {
  ...
  const groupRecords = useCallback((recordsToGroup: GlobalSalesTeam[]) => {
    const groups = new Map<string, GlobalSalesTeam[]>();
