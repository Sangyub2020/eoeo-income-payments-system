...
type SortField = 'category' | 'companyName' | 'projectName' | 'expectedDepositDate' | 'expectedDepositAmount' | 'depositDate' | 'depositAmount' | null;
type SortDirection = 'asc' | 'desc';

interface GlobalMarketingAdvanceBalanceListProps {
  onSuccess?: () => void;
}

export function GlobalMarketingAdvanceBalanceList({ onSuccess }: GlobalMarketingAdvanceBalanceListProps) {
  ...
  // 선택된 열 관리 (거래 유형, Project Name, 회사명 제외)
  const [visibleColumns, setVisibleColumns] = useState<Set<string>>(
    new Set(allColumns.map(col => col.key).filter(key => 
      key !== 'category' && key !== 'projectName' && key !== 'companyName'
    ))
  );
  ...
  // 선/잔금 경고 체크 함수
  const checkAdvanceBalanceWarning = useCallback((record: GlobalMarketingTeam, allRecords: GlobalMarketingTeam[]): boolean => {
  ...
  // 그룹화 함수: 거래 유형, Project Name, 회사명 기준
  const groupRecords = useCallback((recordsToGroup: GlobalMarketingTeam[]) => {
    const groups = new Map<string, GlobalMarketingTeam[]>();
    
    recordsToGroup.forEach(record => {
      const groupKey = `${record.category || ''}|${record.projectName || ''}|${record.companyName || ''}`;
