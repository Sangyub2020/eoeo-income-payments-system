...
interface BrandPlanningBulkModalProps {
  isOpen: boolean;
  onClose: () => void;
  onSuccess: () => void;
}

export function BrandPlanningBulkModal({ isOpen, onClose, onSuccess }: BrandPlanningBulkModalProps) {
  const [csvText, setCsvText] = useState('');
  const [records, setRecords] = useState<Partial<BrandPlanningTeam>[]>([]);
  ...
  const parseCsvText = (text: string, categoryToCodeMap: Map<string, string>): Partial<BrandPlanningTeam>[] => {
    const parsedRecords: Partial<BrandPlanningTeam>[] = [];
    ...
    try {
      ...
      for (let i = 1; i < allLines.length; i++) {
        ...
        const record: Partial<BrandPlanningTeam> = {
  ...
  const updateRecord = (index: number, updates: Partial<BrandPlanningTeam>) => {
  ...
  const handleSubmit = async () => {
    ...
    try {
      ...
      const response = await fetch('/api/income-records/bulk', {
        ...
        body: JSON.stringify({ team: 'brand_planning', records: recordsToSubmit }),
      });
