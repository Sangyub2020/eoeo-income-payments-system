...
interface BrandPlanningAdvanceBalanceListProps {
  onSuccess?: () => void;
}

export function BrandPlanningAdvanceBalanceList({ onSuccess }: BrandPlanningAdvanceBalanceListProps) {
  ...
  // 선/잔금 경고 체크 함수
  const checkAdvanceBalanceWarning = useCallback((record: BrandPlanningTeam, allRecords: BrandPlanningTeam[]): boolean => {
  ...
  const groupRecords = useCallback((recordsToGroup: BrandPlanningTeam[]) => {
    const groups = new Map<string, BrandPlanningTeam[]>();
