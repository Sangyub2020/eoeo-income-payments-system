...
interface OnlineCommerceAdvanceBalanceListProps {
  onSuccess?: () => void;
}

export function OnlineCommerceAdvanceBalanceList({ onSuccess }: OnlineCommerceAdvanceBalanceListProps) {
  ...
  // 표시할 열만 정의 (비율을 선/잔금 바로 오른쪽으로 이동)
  const allColumns = [
  ...
  // 선/잔금 경고 체크 함수
  const checkAdvanceBalanceWarning = useCallback((record: OnlineCommerceTeam, allRecords: OnlineCommerceTeam[]): boolean => {
    ...
    if (advanceBalance === '선금' || advanceBalance.trim() === '선금') {
      // 같은 (거래 유형, 프로젝트 유형, Project Name, 회사명) 조합에 잔금이 있는지 확인
      ...
      return !hasMatchingRemainder; // 잔금이 없으면 경고
    }
    
    // 잔금인 경우
    if (advanceBalance === '잔금' || advanceBalance.trim() === '잔금') {
      // 같은 (거래 유형, 프로젝트 유형, Project Name, 회사명) 조합에 선금이 있는지 확인
      ...
      return !hasMatchingAdvance; // 선금이 없으면 경고
    }
  ...
  useEffect(() => {
    ...
    filtered = filtered.filter(record => {
      ...
      if (!isAdvance && !isRemainder) return true; // 선금/잔금이 아니면 포함
      ...
      // 같은 (거래 유형, 프로젝트 유형, Project Name, 회사명) 조합 찾기 (일시불 제외된 records 사용)
  ...
  const groupRecords = useCallback((recordsToGroup: OnlineCommerceTeam[]) => {
    const groups = new Map<string, OnlineCommerceTeam[]>();
